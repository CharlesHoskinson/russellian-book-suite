...
Each skill ships its own pytest suite. The knowledge-graph layer is EDN-front/Cozo-back via `pycozo`; `rdflib` remains only for thesis entailment and `audit_taxonomy`. `pyshacl` and `pyDatalog` were removed in v0.4.0. LLM tooling is parameterized — production code accepts a `Callable[[str], str]` so tests pass stub responses.
...
Note: `book-qa` ships a `pyproject.toml` declaring `pyyaml` runtime + `pytest`/`jsonschema` dev; `pip install -e .[dev]` works the same as the other skills.
...
skills/<skill>/          source for each skill (priority for review)
  ...
  tests/                 pytest suites
  assets/                schemas, queries, constraints, templates
...
- `examples/*/graph/reports/**` — generated constraint / propagation reports
