...
A family of nine core Claude Code skills that produces non-fiction books from a local claim-ledger and chapter contracts (`book-knowledge`, `russellian-style`, `feynman-style`, `halmos`, `book-compose`, `book-review`, `review-conductor`, `book-qa`, `book-thesis`), plus an optional verifier-scaffolder skill `neurosym-forge` and a standalone paragraph-threading skill `paragraph-weaver`. See `README.md` for the architecture diagram and skill table. Source of truth for design decisions: `docs/specs/`; source of truth for operator workflows: `docs/operations/`.
