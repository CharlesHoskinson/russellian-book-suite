...
**Now resumable at P5.3** — flip `KG_BACKEND` default to cozo + the 10-point cutover gate. **KNOWN BLOCKER:** the FULL suite isn't cozo-green yet (`run_competency_queries` and the RDF-injection fixtures are rdflib-pinned); P5.3 must flip the default, make every consumer cozo-green, rework/retire the RDF-only fixtures, and assert the 10 gate points (plan task P5.3). Then **P5.4** (delete rdflib/pyshacl/pyDatalog + the parallel pyDatalog pass + `compile_thesis`'s TTL emit; KEEP audit_taxonomy on a narrow no-bypass allowlist).
