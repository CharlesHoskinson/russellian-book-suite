...
**Now resumable at P3.1** — the thesis→Cozo projector (`book-thesis/scripts/project_thesis_cozo.py`): read `thesis/<id>.yaml`, load thesis-node/sub-argument/invariant rows into a `CozoStore` via the P3.0 bridge; may need an `:invariant` entity in `kg-schema.edn`. Then P3.2 (D9–D11 EDN→Cozo consistency + parity vs the frozen C0.3 goldens).
