...
- **P5.4a-2 TODO:** delete `project_graph` + `assets/queries/*.rq`; make `run_competency_queries` cozo-only — REQUIRES migrating the query class/`_meta.yaml` (coverage/consistency/defeasible) that `discover_queries` reads from the `.rq` tree to the EDN side; rework `test_project_graph*` / the run_competency rdflib-leg tests.
