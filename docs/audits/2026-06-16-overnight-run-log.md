...
**Now resumable at P3.2** — port the D9–D11 consistency pass to EDN→Cozo and prove parity against the frozen C0.3 goldens (`book-thesis/tests/golden/consistency/d9_d11_{bermuda,violating}.json`). Inputs now in the store from P3.1: thesis-node/sub-argument/invariant. D10/D11 also need claim subject/value + `implies`/`supports_nodes` edges, which `project_ledger_cozo` does NOT carry today (those are inlined synthetic fields the pyDatalog pass reads from the ledger directly) — P3.2 must decide how to get claim facts into Cozo (extend the claim projection, or a thesis-side claim-fact loader). D10 transitive contradiction is recursive (the recursive-Datalog risk flagged in the plan).
