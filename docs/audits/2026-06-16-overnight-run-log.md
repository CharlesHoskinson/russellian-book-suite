...
**Now resumable at P5.4 (final)** — the deletion PR: remove `project_graph`'s RDF emit / `shapes.ttl` / `assets/queries/*.rq` / `compile_thesis`'s TTL emit / `rules/consistency.dl` / the rdflib-internals of `validate_shacl` + the pyDatalog `datalog_consistency`; drop `rdflib`/`pyshacl`/`pyDatalog` from the three pyproject.toml; add `test_no_legacy_import_after_cutover` (the no-bypass scan, with a NARROW `audit_taxonomy` allowlist — it stays rdflib). Rework the now-legacy-pinned RDF-injection tests to ledger-based fixtures (or delete them with the rdflib path). KEEP `audit_taxonomy`. Wire the QA gate's consistency pass to `consistency_cozo`. This is the last, bakeable step — human review before deletion.
