...
| P5 | P5.3b cutover gate test | DONE | 7d95f53 | `test_cutover_gate.py`: default backend is cozo for BOTH dispatch points (spied), bermuda PRESENT (hard fail not skip), default-cozo pipeline clean on bermuda (SHACL conforms + competency completes). Per-golden/constraint/consistency/chapter-evidence parity locked in existing suites (not duplicated). No-legacy-import scan deferred to P5.4 (imports intentionally still present). **book-knowledge 324 passed.** |

**Phase P5.3 COMPLETE — Cozo is the default backend, gated.** The legacy rdflib/pyshacl/pyDatalog stack is still PRESENT (revertable) and exercised only by explicitly-pinned `KG_BACKEND=rdflib` legacy-path tests.

**P5.4 IN PROGRESS** on branch `feat/homoiconic-kg-p5.4-deletion` (off main; NOT merged). Recon corrected the scope: **rdflib STAYS** (book-thesis entailment/lint/exemplar layer reads `compile_thesis`'s TTL; `audit_taxonomy`) — so the end-state is "Cozo sole CLAIM store," not "sole store." Split into a/b, executed as small verified-green increments:
- **P5.4a-1 DONE (9bbc150):** removed the pyshacl SHACL stack — `validate_shacl` cozo-only; deleted `shapes.ttl` + the rdflib-injection fixture/test + raw golden; reworked the cozo-leg tests; dropped pyshacl from book-knowledge + book-compose; book-compose seeds drop the shapes copy and the bundle non-conforming test uses a raw sourceless-claim ledger record. **book-knowledge 312; book-compose subset 36.**
- **P5.4a-2 DONE (2134435):** deleted `project_graph` (no production consumer) + `assets/queries/*.rq` + the project_graph tests; `run_competency_queries` cozo-only (discovers from `kg-queries/*.edn` + a new `kg-queries/_meta.yaml` manifest that migrated the class/severity off the `.rq` tree); dropped `_load_dataset`/rdflib-leg/cross-backend tests; `capture_characterization.capture()` now runs the EDN ports over Cozo. **book-knowledge 295; book-compose subset 36; book-thesis 65.**
- **P5.4b TODO:** extract `DefectReport`/`_emit_pairs`/`_value_str` from `datalog_consistency` into `consistency_cozo` (it imports them), delete `datalog_consistency`/`consistency.dl`/`capture_consistency`, wire the QA gate to `consistency_cozo`, drop pyDatalog from book-thesis.
- **Both:** `test_no_legacy_import_after_cutover` (ban pyshacl/pyDatalog + TriG parsing; rdflib allowlisted), venv rebuilds, docs.

**Original P5.4 note (superseded by the above):** the deletion PR: remove `project_graph`'s RDF emit / `shapes.ttl` / `assets/queries/*.rq` / `compile_thesis`'s TTL emit / `rules/consistency.dl` / the rdflib-internals of `validate_shacl` + the pyDatalog `datalog_consistency`; drop `rdflib`/`pyshacl`/`pyDatalog` from the three pyproject.toml; add `test_no_legacy_import_after_cutover` (the no-bypass scan, with a NARROW `audit_taxonomy` allowlist — it stays rdflib). Rework the now-legacy-pinned RDF-injection tests to ledger-based fixtures (or delete them with the rdflib path). KEEP `audit_taxonomy`. Wire the QA gate's consistency pass to `consistency_cozo`. This is the last, bakeable step — human review before deletion.
