# neurosym-forge documentation handoff Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Land three documentation artifacts under `docs/handoffs/` that drive a separate Claude session to produce full neurosym-forge operator docs.

**Architecture:** Pure documentation. Three markdown files: a README establishing the new `docs/handoffs/` convention, a comprehensive work-order brief, and a pasteable prompt. No code changes. Spec at `docs/specs/2026-05-14-neurosym-forge-docs-handoff-design.md`.
...
Read the spec end-to-end: `docs/specs/2026-05-14-neurosym-forge-docs-handoff-design.md`.

**Worktree:** `C:\Users\charl\code\russellian-book-suite-docs-handoff` on branch `spec/forge-docs-handoff`. Spec already committed.

**Commit hygiene:** Per repo CLAUDE.md — terse human commits, no AI attribution, no Co-Authored-By, one problem per commit.
...
├── 2026-05-14-neurosym-forge-documentation-brief.md            work order
└── 2026-05-14-neurosym-forge-documentation-prompt.md           pasteable prompt
```

---

## Task 1: `docs/handoffs/README.md` — establish the convention
...
git commit -m "docs/handoffs: establish convention for cross-session work orders"
```

---

## Task 2: The documentation brief

**Files:**
- Create: `docs/handoffs/2026-05-14-neurosym-forge-documentation-brief.md`
...
```bash
git add docs/handoffs/2026-05-14-neurosym-forge-documentation-brief.md
git commit -m "docs/handoffs: brief for neurosym-forge documentation"
```

---

## Task 3: The pasteable prompt

**Files:**
- Create: `docs/handoffs/2026-05-14-neurosym-forge-documentation-prompt.md`

- [ ] **Step 1: Write the file with this exact content.**

```markdown
You are working in the russellian-book-suite repo at `C:\Users\charl\code\russellian-book-suite`. Pull the latest main before starting.

Your mission: produce full operator-facing documentation for the `neurosym-forge` skill. The complete brief is at `docs/handoffs/2026-05-14-neurosym-forge-documentation-brief.md`. Read it end to end — it is self-contained and tells you the mission, the source materials to read first, the four deliverables with target paths and required sections, the style authority (russellian-style + humanizer), the workflow (brainstorm → plan → subagent execution), and the acceptance criteria.

Use the superpowers stack: `superpowers:brainstorming` to confirm scope, `superpowers:writing-plans` to plan, `superpowers:subagent-driven-development` to execute. Use git worktrees for parallel edits. Branch off `origin/main` to a branch named `docs/neurosym-forge-documentation`. Open a single PR titled `docs: full neurosym-forge documentation` when finished.

Style discipline: every new prose file must pass the `russellian-style` linters with zero critical findings and pass a `humanizer` pass. Commits must be terse and human; no AI attribution; no Co-Authored-By.

Begin by reading the brief.
```

- [ ] **Step 2: Commit.**

```bash
git add docs/handoffs/2026-05-14-neurosym-forge-documentation-prompt.md
git commit -m "docs/handoffs: pasteable prompt for neurosym-forge documentation"
```
...
```bash
cd C:/Users/charl/code/russellian-book-suite-docs-handoff
git push -u origin spec/forge-docs-handoff
```

- [ ] **Step 2: Open the PR.**

```bash
gh pr create --title "docs/handoffs: brief + prompt for neurosym-forge documentation" --body "$(cat <<'EOF'
## Summary

Establishes a `docs/handoffs/` convention and ships the first handoff: a brief plus a pasteable prompt that drives a separate Claude Code session to produce full `neurosym-forge` operator documentation.

- `docs/handoffs/README.md` — the convention (brief + prompt pair, kept in git as templates)
- `docs/handoffs/2026-05-14-neurosym-forge-documentation-brief.md` — work order. Mission, sources, four deliverables (operator runbook, concepts overview, README section, expanded worked-example), style authority (russellian-style + humanizer), workflow, acceptance criteria
- `docs/handoffs/2026-05-14-neurosym-forge-documentation-prompt.md` — ~500-character pasteable text that points the next session at the brief

Spec: `docs/specs/2026-05-14-neurosym-forge-docs-handoff-design.md`.

## Test plan

- [ ] All three files render in the GitHub preview
- [ ] Brief links resolve to real paths in the repo
- [ ] No code changes; CI passes by default (no test regressions possible)
EOF
)"
```
...
|---|---|
| `docs/handoffs/README.md` convention doc | Task 1 |
| Brief comprehensive work order | Task 2 |
| Pasteable prompt | Task 3 |
| Push + open PR | Task 4 |

All three artifacts the spec requires have implementing tasks. Style authority (russellian + humanizer + repo conventions) is embedded in the brief itself (Task 2) — that's the next session's job to apply, not this one's.

**Placeholder scan:** No TBD/TODO in any step. Every file step has the exact verbatim content.

**Type consistency:** N/A — pure markdown, no types.

**Effort:** ~1 hour total per spec estimate.
