...
> **STATUS — SHIPPED & AUDITED GREEN (2026-06-19).** Implemented and merged in `fba1122` ("sprint 4 — booklogic-pr5-bermuda-migration"). Bermuda's `rules/booklogic/` (7 source files), regenerated `axioms.rs`/`kg.rs`, the deleted `canonical.rs`, and the `cargo test --features smt` CI gate are all in place. Audit re-ran the bermuda suite: **72 passed / 2 skipped**. One latent post-merge drift was found and fixed: `axioms.rs` predated the `#230` quantified-predicate codegen change (2026-06-16) and was never re-baked — it lacked the new z3 imports, the `:scope :subject` constraint field, and the `booklogic-schema.edn` the compiler now emits. Re-baked with the pinned Rust 1.90.0 toolchain in `0952562`. The drift was not CI-gated because CI runs Bermuda's `make ci` + `cargo test`, not its pytest suite. Checkboxes are left unticked because the work shipped as squashed sprint commits, not step-by-step against this doc.
