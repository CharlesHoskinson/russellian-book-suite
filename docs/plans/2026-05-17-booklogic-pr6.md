...
**Size:** 8 phases / small task count. Phase 1 (scaffold) and Phases 3-4-7-8 (fixtures, codegen, mission-spec footer, PR) are each tiny. Phase 2 (four BookLogic source files, mostly mechanical from mission-spec text) and Phase 5 (smoke harness handling verdict-shape duality between PR-4 and the older `:status` emit) are the larger pieces. Phase 6 is CI YAML plus local actionlint.
