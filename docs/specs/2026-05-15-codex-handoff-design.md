...
Hand Codex GPT-5.5 a six-phase brief covering a deep audit followed by the four remaining PRs. Codex operates the local Codex CLI (≥0.130) in a dedicated worktree and pushes one branch per phase. Claude opens the PR, files a `PR-N-REVIEW.md`, and merges. Codex needs git push but does **not** need GitHub CLI auth. Codex keeps a running `docs/codex-wiki/` notebook in the repo for design notes, audit findings, lessons learned, and intermediate state between sessions.

## Out of scope

- Cloud Codex / `codex remote-control`. Local CLI only. The repo is private; the user wants the worktree visible.
- Codex opening or merging PRs. Claude does both. The user reads the PR-N-REVIEW.md after the fact.
...
```
Phase 0  bootstrap          ── Codex reads guidance, initializes docs/codex-wiki/, pushes branch
Phase 1  audit              ── Codex applies codex-review-protocol scoped to neurosym-forge + CLJS,
                                 writes docs/codex-wiki/01-audit-findings.md (no code changes), pushes branch
Phase 2  remediation        ── Codex fixes Critical and Important findings only, pushes branch
Phase 3  PR-3.5             ── port Python ingesters to CLJS, pushes branch
Phase 4  PR-4               ── BookLogic active forms, pushes branch
Phase 5  PR-5               ── Bermuda migration + real Z3, pushes branch
Phase 6  PR-6               ── osmotic-pressure showcase, pushes branch
```

Claude opens each PR from Codex's pushed branch, reviews via PR-N-REVIEW.md, and merges. Codex does not start the next phase until the previous PR-N-REVIEW.md merges to main. The user reads the review record but is not in the loop on the merge button.
...
- Sandbox: `workspace-write` (Codex may write inside the worktree, read outside, but no outbound network beyond `npm install` and `git push origin <branch>`).
- Approval: `on-failure` (Codex runs autonomously; surfaces to the user only on shell failures it cannot recover from).
- No `gh` auth required. Codex never invokes `gh`; Claude owns all GitHub-side operations from a separate session.
...
- create branches, push to its own branches via `git push origin <branch>`
...
- open or merge PRs (Claude owns those; Codex has no `gh` auth)
...
- make outbound network calls beyond `npm install` and `git push` (no curl, wget, gh, model API calls, telemetry)
...
Codex finishes a phase by pushing the branch and writing a final commit whose message body carries the PR-body content Claude needs. Format:
...
- ruff results (form: `python -m ruff check <path>`)
```

Claude reads the last commit on the branch, opens the PR with that body verbatim, reviews, files `openspec/changes/<phase>/PR-<N>-REVIEW.md`, and merges. Review feedback rides into the repo on the merged review file; Codex reads it via `git pull origin main` on the next session.

If Codex needs to address review comments after a merge (e.g., the PR-N-REVIEW.md flagged a P1 follow-up), Codex pushes a new branch (`codex/<phase>-fixes`) and the same handoff cycle repeats.

## Failure modes anticipated

- **Codex pushes a branch without QA evidence in the final commit message.** Brief mandates "real QA evidence" in the For-reviewer commit body; Claude opens the PR with `--draft` and a request-changes comment instead of approving.
...
- **Codex's wiki notes drift from reality.** Wiki updates ship in the same branch as the work; if the diff is right and the wiki is wrong, the wiki is what changes.
- **Codex doesn't notice a merged PR-N-REVIEW.md flagged a P1 follow-up.** Brief's resume prompt instructs Codex to read `openspec/changes/<latest-phase>/PR-*-REVIEW.md` at session start; missing this is a process bug Codex must self-correct.

## What the user does

- Reads the audit report (Phase 1 deliverable) and signals go / no-go before Phase 2.
- Reads each PR-N-REVIEW.md after the fact. Trusts Claude's review-and-merge for routine phases.
...
- Opens the PR from Codex's pushed branch. Body is the For-reviewer section from Codex's final commit.
- Reviews each PR per the PR-review-style memory (severity buckets, gate-replay discipline, no nitpicks).
- Files PR-N-REVIEW.md under `openspec/changes/<phase>/` mirroring each PR. Lands the review file as a separate `review:` PR after merging the work PR.
- Merges the PR via `gh pr merge --squash --delete-branch` once approved. If review comments require a fix, posts the review with `--request-changes` and pings the user to re-engage Codex.
- Does NOT implement or push code commits to Codex's branches. Bug fixes in Codex's work are Codex's job on the next session.
