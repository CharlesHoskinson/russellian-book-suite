...
**Architecture:** A deterministic layer (a concept ledger + a linkage computation with two mechanically-decidable critical flags — forward-reference and broken-seam) plus a Halmos-reviewer agent that adjudicates the semantic checks (missed-recall, spiral-stall, continuity-gap, terminology-drift, premature-definition). An aggregator merges both into `halmos-verdict.json`; `book-compose`'s contract check reads `halmos_critical_count` as a gating metric.
...
{"chapter_id": "ch-09", "n": 9,
 ...
 "seam": {"prev_close": "...", "this_open": "...", "status": "clean", "overlap": ["airgap","authority"]},
 "flags": [{"check": "forward-reference", "severity": "critical", "concept": "bounded-polis", "detail": "ch-09 references 'Bounded Polis', first introduced in ch-10"}]}
...
def harvest_title_case(text: str) -> list[str]:
    """Distinct Title-Case multi-word phrases, minus stop phrases."""
    seen, out = set(), []
...
Responsibility: for chapter N, compute referenced/introduced concepts, the N−1→N seam status, and the two mechanically-decidable critical flags (`forward-reference`, `broken-seam`). Writes `halmos/linkage/ch-NN.json`.
...
def test_forward_reference_is_critical(tmp_path):
    # ch-07 references "Bounded Polis", but the Polis is first introduced in ch-10.
    ws, seed = _ws(tmp_path, {
        "ch-07": "# C7\nCall it the Authority Airgap. It anticipates the Bounded Polis.\n",
        "ch-10": "# C10\nThe Bounded Polis separates powers.\n",
    })
    build_concept_ledger(ws, seed_path=seed)
    link = build_linkage(ws, "ch-07")
    fr = [f for f in link["flags"] if f["check"] == "forward-reference"]
    assert any(f["concept"] == "bounded-polis" and f["severity"] == "critical" for f in fr)
    # and it is persisted
    p = ws / "halmos" / "linkage" / "ch-07.json"
    assert json.loads(p.read_text(encoding="utf-8"))["flags"]
```
...
STOPWORDS = set("a an the of to in on and or but is are was were be been it its that this "
                "for with as by at from into out up down over under then so".split())
...
def build_linkage(workspace: Path, chapter_id: str) -> dict:
    ...
    for c in concepts:
        ...
        if any(f in nbody for f in forms):
            ...
            if c["intro_n"] > n:
                flags.append({
                    "check": "forward-reference", "severity": "critical", "concept": c["slug"],
                    "detail": f"{chapter_id} references '{c['concept']}', first introduced in {c['introduced_in']}",
                })
...
```python
def test_run_halmos_gates_on_forward_reference(tmp_path):
    ws = tmp_path / "ws2"
    data = {
        "ch-07": "# C7\nThe Authority Airgap separates power. It anticipates the Bounded Polis.\n",
        "ch-10": "# C10\nThe Bounded Polis separates the powers.\n",
    }
    for cid, body in data.items():
    ...
    assert verdict["halmos_critical_count"] == 1     # deterministic forward-reference to Bounded Polis
...
Expected: all green (concept ledger 3, linkage 3, aggregate 2, integration 3).
