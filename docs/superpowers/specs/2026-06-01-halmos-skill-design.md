...
1. **`scripts/concept_ledger.py`** — maintains `halmos/concepts.jsonl`. Each record is one
   concept: `{concept, slug, gloss, introduced_in, intro_n, aliases[], source: seed|harvested}`.
   Concepts are harvested from two places:
   - a **curated seed list** (`references/seed-concepts.txt`): book devices and their aliases,
     stamped `source: seed`;
   - a **named-device extractor** over each chapter draft: capitalized multi-word terms and
     ...
     "logic monopoly") that recur across ≥2 chapters, stamped `source: harvested`, with a
     one-line gloss taken from the sentence that introduces them.
   > **v0.1 scope:** harvesting from the verified claim ledger (`book-knowledge.ledger`,
   > `source: claim`) is deferred. v0.1 ships device-only concepts (seed + harvested).
...
2. **`scripts/build_linkage.py`** — for chapter N, computes the deterministic linkage and
   ...
   - `flags`: the one mechanically-decidable critical, `broken-seam`. The semantic checks
     (`orphan-reference`/forward-reference, `terminology-drift`, `missed-recall`,
     `spiral-stall`, `continuity-gap`) are NOT decidable deterministically — a concept that
     appears in N necessarily has `intro_n ≤ N`, so "referenced before introduced" cannot be
     computed from the ledger. The agent owns them, using the `references`/`introduces`
     inventory and the priors digest. The deterministic layer supplies the inventory and the
     seam; the agent supplies the linkage judgments.
