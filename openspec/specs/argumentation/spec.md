# Capability: argumentation

The argumentation capability derives Dung grounded acceptability over the existing argument edges and emits writer warnings (contested load-bearing, axiom-only support, unsupported load-bearing) with bounded justifications. First defined by the `kg-argumentation-layer` change (archived).
