# Capability: claim-first-drafting

The claim-first-drafting capability is the live book-compose drafting loop driven by the knowledge graph: the chapter draft is scaffolded from the S1 chapter-retrieval bundle (claim-first, citation-first), not a flat claim list. First defined by the `live-chapter-bundle-input` change (archived).
