...
- **conformance harness** — the dual-run test rig behind `cozo_store` that runs a
  frozen EDN query fixture against both backends and asserts equality. *(S8)*
- **reference backend** — a small, independent EDN/Datalog evaluator covering a
  declared rule subset; authoring-time/test only, never the production store, and
  it imports no `pycozo`. *(S8)*
- **canonical ordering** — the fixed, backend-independent total order over result
  rows (multiset-preserving) under which two result sets are compared. *(S8)*
- **switch-trigger** — a documented condition under which a backend swap is
  reconsidered, not performed automatically. *(S8)*
