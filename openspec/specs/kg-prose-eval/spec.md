# Capability: kg-prose-eval

The kg-prose-eval capability grades the prose pipeline against frozen, immutable benchmark tasks (ledger snapshot + chapter contract + curated gold) with deterministic, golden-pinned metrics. First defined by the `kg-prose-eval-harness` change (archived).
