...
def test_violating_workspace_emits_triples(tmp_path):
...
def test_violating_workspace_does_not_conform(tmp_path):
    layout = build_violating_workspace(tmp_path)
...
def test_confidence_range_violation_present(tmp_path):
...
def test_verified_without_source_violation_present(tmp_path):
...
def test_chapter_section_violation_present(tmp_path):
...
def test_fixture_is_deterministic_across_tmp_paths(tmp_path):
