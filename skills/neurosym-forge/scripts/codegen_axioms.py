...
def _rust_string_literal(s: str) -> str:
    """A safe double-quoted Rust string literal for an arbitrary Python str.

    Names and values reaching the codegen come from claim text. Escaping the
    backslash, double-quote, and common control characters keeps a predicate,
    subject, or value containing `"`/`\\`/newline from breaking out of the
    emitted Rust literal (REQ-NSI-002 / H-02). Other characters (including
    non-ASCII) are valid as-is in a UTF-8 Rust source string.
    """
    body = (str(s).replace("\\", "\\\\")
            .replace('"', '\\"')
            .replace("\n", "\\n")
...
def _emit_egg_block(c: dict) -> str:
    ...
    return (
        ...
        f'        let tracker = Bool::new_const({_rust_string_literal(str(cid))});\n'
...
def _emit_vector_const(var_name: str, inner_type: str) -> str:
    ...
    return f'Array::<Int, {inner_type}>::new_const({_rust_string_literal(var_name)})'
...
def _emit_set_const(var_name: str, inner_type: str) -> str:
    ...
    return f'Set::<{inner_type}>::new_const({_rust_string_literal(var_name)})'
...
def _emit_expr_typed(node: Any, z3_type: str, bound_vars: dict[str, str] | None = None) -> str:
    ...
    if isinstance(node, Keyword):
        return f'Z3String::from_str({_rust_string_literal(node.name)}).expect("valid utf-8")'
    if isinstance(node, str):
        return f'Z3String::from_str({_rust_string_literal(node)}).expect("valid utf-8")'
    ...
    if isinstance(node, (list, tuple, EdnList, EdnVector)) and len(node) > 0:
        ...
        if isinstance(head, Keyword):
            ...
            if info is not None:
                ...
                lit = _rust_string_literal(var_name)
                if z3_type == "Bool":
                    return f'Bool::new_const({lit})'
                if z3_type == "Real":
                    return f'Real::new_const({lit})'
                if z3_type == "Z3String":
                    return f'Z3String::new_const({lit})'
                return f'Int::new_const({lit})'
        head_str = str(head)
        if head_str in {"*", "+", "-"} and len(node) >= 3:
...
def _emit_bool_subexpr(
    ...
    if isinstance(head_node, Keyword):
        ...
        return f'Bool::new_const({_rust_string_literal(var_name)})'
    raise CodegenError(
...
def _emit_expr(node: Any, bound_vars: dict[str, str] | None = None) -> str:
    ...
    if isinstance(node, (list, tuple, EdnList, EdnVector)) and len(node) > 0:
        ...
        if isinstance(head, Keyword):
            ...
            return f'Int::new_const({_rust_string_literal(var_name)})'
        # (* a b ...) / (+ ...) / (- a b)
        head_str = str(head)
        if head_str in {"*", "+", "-"} and len(node) >= 3:
