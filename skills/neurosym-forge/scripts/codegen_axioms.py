...
def generate_axioms_source(constraints: list[dict]) -> str:
    ...
    for c in constraints:
        ...
        if backend == Keyword("z3"):
            ...
            # smoke time. Add the (name, datalog) pair to a sibling
            # registry that lib.rs runs through kg::evaluate_constraint;
            # the Z3 entry point sees nothing.
            cozo_entries.append(_emit_cozo_block(c))
        else:
            # :egg constraints still flow through eqsat.rs.
            continue
