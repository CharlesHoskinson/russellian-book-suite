...
def test_apply_revisions_happy_path(tmp_path):
    ...
    result = apply_revisions(chapter_path=chapter, revisions_obj=revisions_obj, output_path=out_path)
    assert result == 1
...
def test_apply_revisions_missing_original_raises(tmp_path):
    ...
    # Side effect: the single-revision failure is added to unresolved
    assert any(u["cluster_id"] == "C01" for u in revisions_obj["unresolved"])
...
def test_apply_revisions_tolerates_partial_failure(tmp_path):
    """When some revisions fail and some succeed, succeed ones are applied;
    failures are added to revisions_obj['unresolved']; no raise."""
    chapter = tmp_path / "ch.md"
    chapter.write_text(
        "Paragraph one.\n\nParagraph two has a listicle abstract.\n\nParagraph three.\n",
        encoding="utf-8",
        ...
        "revisions": [
            {
                "cluster_id": "C01",
                "original": "Paragraph two has a listicle abstract.",
                "revised": "Paragraph two argues in prose.",
                "rationale": "C01 fix",
            },
            {
                "cluster_id": "C02",
                "original": "Paragraph that does NOT exist.",
                "revised": "anything",
                "rationale": "C02",
            },
        ],
    ...
    applied = apply_revisions(
        chapter_path=chapter, revisions_obj=revisions_obj, output_path=out_path
    )
    assert applied == 1
    revised = out_path.read_text(encoding="utf-8")
    assert "Paragraph two argues in prose." in revised
    assert "Paragraph one." in revised
    assert "Paragraph three." in revised
    # C02 failure added to unresolved (with the apply-specific reason)
    assert any(
        u["cluster_id"] == "C02" and "verbatim match" in u["reason"]
        for u in revisions_obj["unresolved"]
    )


def test_apply_revisions_raises_only_when_all_fail(tmp_path):
    """When EVERY revision fails, ApplyError raises (genuine halt case)."""
    chapter = tmp_path / "ch.md"
    ...
    out_path = tmp_path / "revised.md"
    with pytest.raises(ApplyError, match="all 2"):
        apply_revisions(
            chapter_path=chapter, revisions_obj=revisions_obj, output_path=out_path
        )
    # Output file not created (all-failure halt case)
