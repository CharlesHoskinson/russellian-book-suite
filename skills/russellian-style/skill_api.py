"""
Public API surface of russellian-style (IF-RS-1).
"""
...
@dataclass
class LintIssue:
    linter: str    # rule name, e.g. "no-hedging"
...
_LINTER_REGISTRY: dict[str, tuple[str, str]] = {
    "no-hedging":                   ("scripts.lint_hedges",        "lint_hedges"),
    "active-voice":                 ("scripts.lint_passive_voice",  "lint_passive_voice"),
    "signal-density":               ("scripts.lint_signal_density", "lint_signal_density"),
    "parallel-structure":           ("scripts.lint_parallel_structure", "lint_parallel_structure"),
    "listicle-abstract":            ("scripts.lint_listicle_abstract", "lint_listicle_abstract"),
    "listicle-anaphora":            ("scripts.lint_listicle_abstract", "lint_listicle_abstract"),
    ...
    "burstiness":                   ("scripts.lint_burstiness",     "lint_burstiness"),
    "ai-vocabulary":                ("scripts.lint_ai_vocabulary",  "lint_ai_vocabulary"),
    "staccato-paragraph-run":       ("scripts.lint_ai_staccato",    "lint_ai_staccato"),
    "negation-affirmation-template":("scripts.lint_ai_staccato",    "lint_ai_staccato"),
    "this-is-conclusion-overuse":   ("scripts.lint_ai_staccato",    "lint_ai_staccato"),
    "abstract-subject-run":         ("scripts.lint_ai_staccato",    "lint_ai_staccato"),
    "concrete-instance-density":    ("scripts.lint_concrete_instance_density",
                                     "lint_concrete_instance_density"),
    "epistemic-precision":          ("scripts.lint_epistemic_precision", "lint_epistemic_precision"),
    "paragraph-motion":             ("scripts.lint_paragraph_motion", "lint_paragraph_motion"),
}

# Canonical set of linters to run when no subset is requested.
# We exclude linters that require large corpora or network access.
...
def _raw_findings_for_rule(rule: str, path: Path) -> list[dict]:
    entry = _LINTER_REGISTRY.get(rule)
    if entry is None:
        return []
    module_name, func_name = entry
    try:
        fn = _import_linter(module_name, func_name)
        raw = fn(path)
        return [r for r in raw if r.get("rule") == rule]
    except Exception:
        return []
...
def lint_fragment(text: str, linters: Optional[list[str]] = None) -> list[LintIssue]:
    ...
    try:
        ...
        # (e.g. lint_ai_staccato covers multiple rules) only run once.
