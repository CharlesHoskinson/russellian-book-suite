...
def build_generation_prompt(topic: str, *, mode: str, exemplars: list[dict[str, Any]], guide: str) -> str:
    ...
    return (
        ...
        f"THE GUIDE:\n{guide}\n\n"
        "Open with HOSKINSON (candor, direct address, momentum; state the stakes plainly). Develop "
        "with FEYNMAN (a concrete analogy, build the intuition from scratch, show WHY before any "
        "formalism, narrative warmth). Close with RUSSELL (compress to one exact, declarative, "
        "load-bearing claim; drop every hedge). The discipline floor still applies: no hedging, "
        "active voice, high signal density.\n\n"
