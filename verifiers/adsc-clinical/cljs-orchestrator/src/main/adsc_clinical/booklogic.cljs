...
(defn- valid-return-sort?
  "REQ-DSL-050: a return-sort is either a sort keyword, or a one-layer
   container `[:vector <sort-keyword>]` / `[:set <sort-keyword>]`."
  [ret]
  (or (keyword? ret)
      (and (vector? ret)
           (= 2 (count ret))
...
(defn- return-inner-sort
  "Strip the container wrapper from a multi-valued return sort.
   Scalar return-sorts pass through unchanged."
  [ret]
...
(defn- expand-defpredicate
  "(defpredicate :name [sorts...] return-sort) → {:name :arg-sorts [...] :return ...}

   REQ-DSL-050: return-sort may be a sort keyword OR
   [:vector <sort-keyword>] / [:set <sort-keyword>]."
  [form]
  (let [[_ name args ret] form]
    ...
    (when-not (vector? args)
      (throw (ex-info "defpredicate: args must be a vector of sorts"
                      {:form form})))
    (when-not (valid-return-sort? ret)
      (throw (ex-info (str "defpredicate " name
                           ": return must be a sort keyword or "
                           "[:vector <sort>] / [:set <sort>]")
                      {:form form :return ret})))
    {:name name :arg-sorts args :return ret}))
...
(defn- validate-predicate-sorts
  "Predicates must reference sorts declared in sorts.edn OR primitive
   ...
   REQ-DSL-050: a multi-valued return-sort (`[:vector T]`/`[:set T]`)
   validates its inner element sort the same way as a scalar return."
  ...
  (let [declared-sort-names
    ...
    (doseq [pred predicate-registry
            :let [{nm :name as :arg-sorts r :return} pred]]
      (doseq [a as]
        (when-not (known? a)
          (throw (ex-info (str "defpredicate " nm ": unknown sort " a)
                          {:predicate pred :unknown a}))))
      (let [inner (return-inner-sort r)]
        (when-not (known? inner)
          (throw (ex-info (str "defpredicate " nm ": unknown return sort " r)
                          {:predicate pred :unknown r})))))))
...
(defn- assert-form-approx?
  "True iff the :assert form uses the approx= (approximate-equality) head.
   ...
   EDN symbol); `~=` is accepted as an alias for back-compat. adsc's own
   rules/booklogic/constraints.edn documents and uses `approx=`, so the
   recogniser must accept it or the constraint's :tolerance is silently
   dropped from codegen."
  [assert-form]
...
;; ----- CLI entry: nbb -m adsc-clinical.booklogic <project-root> -----

(defn -main
  "CLI: nbb -m adsc-clinical.booklogic <project-root>
