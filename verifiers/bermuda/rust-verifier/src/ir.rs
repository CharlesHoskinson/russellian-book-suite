...
mod tests {
    ...
    fn emit_verdict_serialises_graph_summary_when_present() {
        ...
        assert!(
            edn.contains(":graph-summary"),
            "graph-summary must be emitted; got {edn}"
        );
    ...
    fn emit_verdict_omits_graph_summary_when_absent() {
        ...
        assert!(
            !edn.contains(":graph-summary"),
            "graph-summary must be omitted when None; got {edn}"
        );
        assert!(edn.ends_with("]}"), "verdict must close cleanly; got {edn}");
    }
}
