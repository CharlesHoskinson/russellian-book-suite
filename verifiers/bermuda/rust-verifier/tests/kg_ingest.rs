...
// the fix `verdict.verified` was always empty, so `claim_count` was 0 and
// the Q001 contradiction query could never fire. This drives the real
// population path (parse -> check_all -> verified) rather than hand-made
// claims.
#[cfg(feature = "smt")]
#[test]
fn check_all_populates_verified_and_kg_sees_real_claims() {
    use bermuda_verifier::ir::parse_formulas;
