...
from scripts._edn_reader import EdnList, EdnVector, Keyword  # noqa: E402
from scripts._io import read_edn_file  # noqa: E402


def _is_seq(x) -> bool:
    """Return True if x is any EDN sequence type (list, EdnList, EdnVector)."""
    return isinstance(x, (list, EdnList, EdnVector))
...
def test_sorts_edn_has_required_sorts() -> None:
    ...
    declared = {f[1] for f in forms if _is_seq(f) and len(f) >= 2}
...
def test_predicates_edn_has_five_existing() -> None:
    forms = _read_forms("predicates.edn")
    names = {f[1] for f in forms if _is_seq(f) and len(f) >= 2}
    expected = {
...
def test_predicates_edn_arity_shape() -> None:
    ...
    forms = _read_forms("predicates.edn")
    bad = [f for f in forms if not (_is_seq(f) and len(f) == 4)]
...
def test_lifts_edn_covers_every_predicate() -> None:
    ...
    preds = {f[1] for f in _read_forms("predicates.edn") if _is_seq(f)}
    lift_targets = set()
    for f in _read_forms("lifts.edn"):
        ...
        if _is_seq(emit) and len(emit) >= 4 and str(emit[0]) == "fact":
...
def test_rules_edn_form_heads_are_defrule() -> None:
    forms = _read_forms("rules.edn")
    bad = [str(f[0]) for f in forms if _is_seq(f) and str(f[0]) != "defrule"]
...
def test_constraints_edn_covers_canonical_facts() -> None:
    ...
    names = {str(f[1]) for f in _read_forms("constraints.edn") if _is_seq(f)}
    expected_canonical = {
