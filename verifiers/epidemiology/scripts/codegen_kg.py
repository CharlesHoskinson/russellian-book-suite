"""Scaffolded entry-point for neurosym-forge codegen_kg.

`scaffold_project.py` copies the canonical library alongside this shim
as `scripts/_codegen_kg_lib.py` (and its EDN reader/writer deps).
Run as:

    python scripts/codegen_kg.py
"""
...
def _import_run():
    ...
    if (_scripts_dir / "_codegen_kg_lib.py").exists():
        from scripts._codegen_kg_lib import run as _run
        return _run
...
def main() -> int:
    run = _import_run()
    run(_project_root)
    print(f"[codegen-kg] {_project_root}/rust-verifier/src/kg.rs regenerated")
    return 0
